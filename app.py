...
# 규격상세 파싱: 300*500*18 / 300x500x18 / 300X500X18 / 300×500×18 등 지원
_spec_pat = re.compile(
    r"^\s*(\d+(?:\.\d+)?)\s*(?:\*|x|X|\u00D7)\s*(\d+(?:\.\d+)?)\s*(?:\*|x|X|\u00D7)\s*(\d+(?:\.\d+)?)\s*$"
)

def parse_spec(s):
    """규격상세: 가로*세로*두께 (구분자: *, x, X, \u00D7)"""
    if pd.isna(s):
...
# 계산 로직
# ======================
def compute_base(df_raw: pd.DataFrame, cfg: dict):
    """업로드 데이터 정리(Data) + 생산일별 순부품면적 요약"""
    df = df_raw.copy()
    ...
    if df[["폭_mm", "길이_mm", "두께_mm"]].isna().any().any():
        raise ValueError("규격상세 파싱 실패 행이 있습니다. 규격상세는 '가로*세로*두께' (예: 300*500*18, 300x500x18, 300\u00D7500\u00D718) 형식이어야 합니다.")
...
def to_excel_bytes(df_data, df_sum_final, df_ana, cfg: dict, usable_area_m2: float):
    output = io.BytesIO()
...
with st.sidebar:
    ...
    st.caption("자투리 이월: 기말 = 기초 + 자투리입고 - 자투리사용 (재고 음수 불가)")
...
usable_h = cfg["board_h_mm"] - 2 * cfg["margin_mm"]
usable_area_m2 = (usable_w * usable_h) / 1_000_000
st.caption(f"유효 원장 면적: {usable_w}×{usable_h}mm = {usable_area_m2:.4f} m²")  # 표시용(×는 화면에서만)
...
try:
    ...
    df_data, df_sum_base, usable_area_m2 = compute_base(df_raw, cfg)
...
with tab1:
    ...
    st.write("""
- **실원장수(장)**: 실제 사용 원장수를 입력하면 그 값을 기준으로 사용면적/로스가 재계산됩니다. (미입력 시 기본원장수 적용)
- **자투리입고(m²)**: 금일 발생/보관 자투리(재고 증가)
- **자투리사용(m²)**: 금일 보관 자투리 사용(재고 감소) — 재고 초과 입력 시 자동으로 가능한 만큼만 적용됩니다.
""")

    edit_df = df_sum_base[[
        "생산일", "순부품면적_m2", "기본원장수_장", "실원장수_장",
        "자투리입고_m2", "자투리사용_m2"
    ]].copy()
    ...
    sum_df_in = df_sum_base.copy()
    sum_df_in["실원장수_장"] = edited["실원장수_장"]
    sum_df_in["자투리입고_m2"] = edited["자투리입고_m2"]
    sum_df_in["자투리사용_m2"] = edited["자투리사용_m2"]

    df_sum_final = apply_overrides_and_inventory(sum_df_in, usable_area_m2, opening_scrap)
    ...
    st.dataframe(
        show_sum[[
            ...
            "기본로스_m2",
            "자투리기초_m2", "자투리입고_m2", "자투리사용_적용_m2", "자투리기말_m2",
            "조정로스_m2", "로스율(%)"
        ]],
        use_container_width=True,
        hide_index=True,
...
with tab2:
    ...
    st.write("생산일별 사용면적·자투리사용(적용)을 해당 일자 내 순면적 비중으로 배분하여 그룹별 로스율을 계산합니다.")
...
excel_bytes = to_excel_bytes(df_data, df_sum_final, df_ana, cfg, usable_area_m2)
st.download_button(
...
st.caption("※ 면적기반 + 실원장수 우선 적용. 커프(날물) 기반 절단 손실은 재단 플랜 데이터가 있어야 정밀 반영 가능합니다.")
